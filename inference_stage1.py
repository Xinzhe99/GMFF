...
"""
A prediction script for StackMFF V4 network.

Usage:
    python inference_stage1.py --input_dir /path/to/image/stack --output_dir /path/to/output --model_path /path/to/model.pth
    python inference_stage1.py --input_dir /path/to/image/stack --output_dir /path/to/output --model_path /path/to/model.pth --save_all

Example:
    python inference_stage1.py --input_dir ./test_images --output_dir ./results --model_path ./best_model.pth
    python inference_stage1.py --input_dir ./test_images --output_dir ./results --model_path ./best_model.pth --save_all

The input directory should contain image files (e.g., 0.jpg, 1.jpg, 2.jpg, ...)
By default, only the final color fused image is saved with the same name as the input folder and extension as the input images.
If --save_all is specified, the following outputs will be saved:
    - fused_gray_*.png: Grayscale fused result
    - focus_indices_*.png: Focus index map visualization
    - color_fused_*.png: Color fused result
    - focus_colormap_*.png: Colormap visualization of focus indices
"""
...
def gray_to_colormap(img, cmap='rainbow'):
    ...
    map_obj = cm.ScalarMappable(norm=norm, cmap=cmap_m)
...
def parse_args():
    ...
    parser.add_argument('--save_all', action='store_true', default=False,
                       help='Save all output images (overrides --only_fused)')
...
def load_image_stack(input_dir):
    ...
    return torch.stack(gray_tensors), color_images, image_files[0] if image_files else None
...
def main():
    ...
    # Load image stack
    image_stack, color_stack, sample_filename = load_image_stack(args.input_dir)
    ...
    # Handle only_fused option
    if args.only_fused and not args.save_all:
        # Get input folder name for output filename
        input_folder_name = os.path.basename(os.path.normpath(args.input_dir))
        
        # Get extension from sample filename
        if sample_filename:
            ext = os.path.splitext(sample_filename)[1]
        else:
            ext = '.png'  # default extension
            
        # Save only the color fused image with the same name as input folder
        output_path = os.path.join(args.output_dir, input_folder_name + ext)
        cv2.imwrite(output_path, color_fused)
        print(f"Only color fused image saved to {output_path}")
    else:
        # Save all results with timestamp (original behavior)
