...
class GMFFPipeline(Pipeline):
    def apply_cleaner(
        self, lq: torch.Tensor, tiled: bool, tile_size: int, tile_stride: int
    ) -> torch.Tensor:
        # Since we're using IdentityCleaner, no actual processing is needed
        # But we still need to support tiled inference
        if tiled and (lq.size(2) < tile_size or lq.size(3) < tile_size):
            print("[GMFF]: the input size is tiny and unnecessary to tile.")
            tiled = False

        # For IdentityCleaner, we don't need to actually apply any model
        # Just work with the input directly
        if tiled:
            # Even with tiling enabled, IdentityCleaner doesn't need special tiling
            # We can just use the input as is
            output = lq
        else:
            output = lq
            
        # Ensure the output meets the minimum size requirement
