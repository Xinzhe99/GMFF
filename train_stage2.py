...
from gmff.model import ControlLDM, Diffusion
from gmff.utils.common import instantiate_from_config, to, log_txt_as_img
from gmff.sampler import SpacedSampler
...
def main(args) -> None:
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            # 从字典中提取数据
            aif = batch["aif"]
            aif_missing = batch["aif_missing"]
            prompt = batch["prompt"]
            aif = rearrange(aif, "b h w c -> b c h w").contiguous().float() #[-1,1]
            aif_missing = rearrange(aif_missing, "b h w c -> b c h w").contiguous().float() #[0,1]

            with torch.no_grad():
                z_0 = pure_cldm.vae_encode(aif)
                cond = pure_cldm.prepare_condition(aif_missing, prompt) #[-1,1 for vae]
                # noise augmentation
            ...
            if global_step % cfg.train.image_every == 0 or global_step == 1:
                ...
                with torch.no_grad():
                    z = sampler.sample(
                        ...
                        device=device,
                        steps=50,
                        x_size=(len(log_aif), *z_0.shape[1:]),
                        cond=log_cond,
                        uncond=None,
                        cfg_scale=1.0,
                    ...
                    if accelerator.is_main_process:
                        for tag, image in [
                            ...
                            ("image/condition", log_aif_missing),
                            (
